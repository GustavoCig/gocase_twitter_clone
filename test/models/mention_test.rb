...
class MentionTest < ActiveSupport::TestCase
  test 'mention with invalid/null parameters shouldnt be stored in the database' do
  ...
  test 'user self mentioning shouldnt generate a mention' do
    user_1 = User.first
  ...
  test 'there shouldnt be more than one unique pair of tweet and user in the database' do
    user_1 = User.first
    user_2 = User.find(2)
